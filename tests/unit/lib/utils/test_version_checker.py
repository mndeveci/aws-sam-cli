...
class TestVersionChecker(TestCase):
    ...
    @patch("samcli.lib.utils.version_checker.LOG")
    @patch("samcli.lib.utils.version_checker.installed_version", "1.9.0")
    def test_compare_current_versions_different(self, mock_log, get_mock):
        ...
        mock_log.assert_has_calls(
            [
                call.debug("Installed version %s, current version %s", "1.9.0", "1.10.0"),
                call.info("There is a newer version available for SAM CLI!"),
                call.info("New version: %s Your version: %s", "1.10.0", "1.9.0"),
                call.info("To download the new version, go here %s", AWS_SAM_CLI_INSTALL_DOCS),
            ]
        )
    ...
    def test_last_check_time_week_earlier_should_return_false(self):
        ...
        self.assertFalse(is_last_check_older_then_week(eight_days_ago))
